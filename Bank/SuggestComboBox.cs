...
namespace AutoCompleteComboBox
{
    public class SuggestComboBox : ComboBox
    {
        ...
        public SuggestComboBox()
        {
            ...
            _filterRuleCompiled = s => s.ToLower().Contains(Text.Trim());
        ...
        protected override void OnTextChanged(EventArgs e)
        {
            ...
            {
                string val=Convert.ToString((item as DataRowView).Row[DisplayMember]);
                if (val.ToLower().Contains(Text.ToLower().Trim()))
                {
                    _suggBindingList.Add(val);
                }

                //var per = item as Person;

                //if (per.Name.Contains(Text.Trim()))
                //{
                //    _suggBindingList.Add(per.Name);
                //}
