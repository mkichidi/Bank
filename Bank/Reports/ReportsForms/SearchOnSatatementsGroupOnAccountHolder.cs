...
{
    public partial class SearchOnSatatementsGroupOnAccountHolder : Form
    {
        public SearchOnSatatementsGroupOnAccountHolder()
        {
            InitializeComponent();
            BindDropdowns();
        }
        ...
        public DataTable BindGrid()
        {
            ...
            SqlCommand cmd = new SqlCommand("BankSearchon", con);
            ...
            DataTable dtDistinctNickName = dataTable.DefaultView.ToTable(true, "NickName");

            foreach (DataRow dr in dtDistinctNickName.Rows)
          {
              // Get all DataRows where the name is the name you want.
              IEnumerable<DataRow> rows = dataTable.Rows.Cast<DataRow>().Where(r => r["NickName"].ToString() == dr["NickName"].ToString());

              DataTable dtInter = rows.CopyToDataTable();

              DataTable dtDistinctAccount = dtInter.DefaultView.ToTable(true, "AccountHolderName");

              string accountName = string.Empty;

              foreach (DataRow drAccountName in dtDistinctAccount.Rows)
              {
                  accountName += drAccountName["AccountHolderName"].ToString()+",";
              }

              accountName = accountName.Substring(0, accountName.Length - 1);

              // Loop through the rows and change the name.
              rows.ToList().ForEach(r => r.SetField("AccountHolderName", accountName));
